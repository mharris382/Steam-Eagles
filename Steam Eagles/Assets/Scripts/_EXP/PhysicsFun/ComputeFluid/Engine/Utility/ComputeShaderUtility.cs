...
namespace _EXP.PhysicsFun.ComputeFluid.Engine
{
    ...
    public static class ComputeShaderUtility
    {
        ...
        public static DispatchDimensions CheckGetDispatchDimensions(ComputeShader shader, int handle,
            uint desired_threadNum_x, uint desired_threadNum_y, uint desired_threadNum_z)
        {
                ...
                desired_threadNum_z % group_size_z != 0)
            {
                Debug.LogError(string.Format(
                    "ERROR: Shader {0}, on kernel {1}, has mismatched desired thread numbers ({2},{3},{4}) and thread groupd sizes ({5},{6},{7}). The desired thread " +
                    "number needs to be a multiply of the group size. Either change the desired thread number or change the " +
                    "group size in the shader.", shader.name, kernelName, desired_threadNum_x, desired_threadNum_y, desired_threadNum_z, group_size_x, group_size_y, group_size_z));
                Debug.Break();
            }
        ...
        public static int GetKernelHandle(ComputeShader cp, string name)
        {
            ...
            else
            {
                ...
                map_kernlesToNames.Add(cp,
                    cp_kernles); // Add this dictionary to the other when you create it. Since this is a reference type, you dont need to add this if you have already done it
            }
