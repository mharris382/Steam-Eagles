	...
	where T : SplineBase
{
	...
	public void OnSceneGUI()
	{
		...
		if (_selectedVertex != -1 && _selectedVertex < _spline.VertexCount)
		{
			...
			var newPoint = Handles.FreeMoveHandle(pointIn,HandleUtility.GetHandleSize(pointIn) * .1f, Vector3.zero, Handles.DotHandleCap);
			if (newPoint != pointIn) MoveVertex(_selectedVertex, splineTransform.InverseTransformPoint(newPoint));
		}
